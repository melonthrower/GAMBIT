...
def aitw_2_uitars(aitw_action: dict):
    ...
    if ex_action_type in ["click", "scroll"]:
    ...
    elif ex_action_type == "press_back":
        return f"press_back()"
    
    elif ex_action_type == "press_home":
        return f"press_home()"
    
    elif ex_action_type == "type":
        return f"type(content='{aitw_action['result_action_text']}')"
    
    elif ex_action_type == "complete":
        return f"finished()"
    
    elif ex_action_type == "impossible":\
        return f"finished()"
    
    elif ex_action_type == "long_press":
        lift_yx = json.loads(aitw_action['result_lift_yx'])
        touch_yx = json.loads(aitw_action['result_touch_yx'])
        click_y, click_x = lift_yx[0], lift_yx[1]
        click_x = int(click_x)
